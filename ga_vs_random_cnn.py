...
from shellgenerator.ga_bypass_cnn_train import ga_bypass_cnn_once
...
model.load_weights('model/cnn_weights.keras')  # 初始化为最原始的模型权重
...
def time_task_ga():
    ...
    while 1:
        ...
        for _ in range(opt_time):
            ...
            ga_code.append(ga_bypass_cnn_once(
        ...
        for v_shell in train_orig_shell_li:
            ...
            for v_code in ga_code:
                ...
                try:
                    ga_score = cnn_logit(tovector(ga_shell).reshape(
    ...
    while 1:
        ...
        for v_shell in train_orig_shell_li:
            ...
            for v_code in rand_code:
                ...
                try:
                    rand_score = cnn_logit(tovector(rand_shell).reshape(
                        1, time_step * embedding_size+low_feature_dim))[0][0]
                except Exception:
                    rand_score = 0
...
if __name__ == '__main__':
    ...
    with open('result/vs_cnn.txt', 'w') as fw:
