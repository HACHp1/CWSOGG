...
heat_map_dir = 'result/lstm.png'  # 热图路径

n_inputs = embedding_size  # 输入维度，等于embedding大小
n_steps = time_step

# --参数区
n_hidden_units = 250  # 隐藏层层数（不一定等于时序长度）
n_classes = 2
...
def build_model(lr=0.001):
    ...
    lstm_out, hidden_state, cell_state = layers.LSTM(
        n_hidden_units,activation='sigmoid', return_state=True)(h1)  # 只取最后一个状态作为输出

    h2_con = layers.Concatenate()([lstm_out, inputs_low])  # 拼接低维特征

    h3 = layers.Dense(n_classes, input_shape=(
        n_hidden_units,))(h2_con)
...
def train_lstm(
        ...
        IS_GAN=False,
        GAN_DIR='model/lstm_weights.keras'
    # VAL_SIZE=VAL_SIZE,
):
    '''
    训练lstm神经网络
    ...
    if IS_GAN:
        ...
        # train_model = tf.keras.models.load_model('model/lstm.keras', custom_objects={
    ...
    # train_model.summary()
    # plot_model(train_model, to_file='result/lstm.png',
