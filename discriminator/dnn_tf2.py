...
heat_map_dir = 'result/dnn.png'  # 热图路径
...
def build_model(lr=0.001):
    ...
    h1 = layers.Dense(n_hidden_units,
                      input_shape=(time_step, embedding_size))(inputs)

    h2 = layers.Flatten()(h1)  # 展开后使用全连接

    h2_con = layers.Concatenate()([h2, inputs_low])  # 拼接低维特征

    h3 = layers.Dense(n_classes, input_shape=(time_step*n_hidden_units,))(h2_con)
...
def train_dnn(
        ...
        IS_GAN=False,
        GAN_DIR='model/dnn_weights.keras'
    # VAL_SIZE=VAL_SIZE,
):
    '''
    训练dnn神经网络
    ...
    if IS_GAN:
        ...
        # train_model = tf.keras.models.load_model('model/dnn.keras', custom_objects={
    ...
    # train_model.summary()
    # plot_model(train_model, to_file='result/dnn.png',
