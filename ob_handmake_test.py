...
if __name__ == '__main__':
    ...
    num_dir = 'result/ob_handmake_num.txt'
    fw = open(num_dir, 'w')  # 清空记录文件
    fw.close()

    fw = open('result/ob_handmake_result.txt', 'w')  # 清空记录文件
    fw.close()
    ...
    x_npy_dir = 'bin/x_obfuscated_hand.npy'
    y_npy_dir = 'bin/y_obfuscated_hand.npy'

    data_dir = 'data/handwrite/'
    ...
    for model_name in list(model_li.keys()):
        eval_model(model_li[model_name], x_test, y_test, 'model: {};data: {}'.format(
            model_name, data_dir), 'result/ob_handmake_result.txt')
