# from .ga_bypass_att_train import ga_bypass_att_train
import pickle
# import numpy
...
# ga_bypass_att_train(1,'bin/ob_ga_code.pkl')
...
def ga_bypass_att_test(orig_shell, save_dir='bin/ob_ga_code.pkl'):
    '''
    ...
    for ga_code in ga_code_li:
        ...
        if good_rate > 0.5:  # 如果良性率大于0.5，则混淆成功，保存好文件
            ...
            with open('data/obfuscated_att/' + temp_file, 'w') as fw:
                fw.write(ob_shell)
            with open('data/obfuscated_bak_att/' + temp_file, 'w') as fw:
                fw.write(ob_shell)
