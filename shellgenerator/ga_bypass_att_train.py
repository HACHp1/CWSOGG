'''
Use GA to bypass attention model
...
def ga_bypass_att_once(orig_shell, max_encode_time, NIND, MAX_GEN):
    '''
    使用orig_shell寻找ga编码
    return ga_code
    '''
    ...
    class MyProblem(ea.Problem):  # 继承Problem父类
        def __init__(self):
            name = 'Shell_attention_encode'  # 初始化name（函数名称，可以随意设置）
            M = 1  # 初始化M（目标维数）
        ...
        def aimFunc(self, pop):  # 目标函数
            ...
            for var_i in Vars:
                ...
                if temp_shell_vec is None:  # 执行失败时直接返回结果为恶意(良性为0.0)
                    ...
                    continue
                # print(temp_shell)
                # print(attention_logit(temp_shell_vec.reshape(
...
def ga_bypass_att_train(train_epoch, save_dir, max_encode_time=10, NIND=NIND, MAX_GEN=MAX_GEN):
    '''
    ...
    for epoch in range(train_epoch):
        ga_code_li.append(
            ga_bypass_att_once(orig_shell_li[epoch %
